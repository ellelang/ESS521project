...
# use the BCR results as the initial population
...
population_bcrseeds <- population
population_bcrseeds
print(population_bcrseeds[[which.max(fitness)]])
fitness_bcrseeds <- fitness
fitness_bcrseeds
#check the number of objects selected
lapply(population_bcrseeds, sum)

FrontRes_seeds <- as.data.frame(t(fitness_bcrseeds))
FrontRes_seeds
...
population_noseeds <- population
population_noseeds
print(population_noseeds[[which.max(fitness)]])
fitness_noseeds <- fitness
fitness_noseeds
#check the number of objects selected
lapply(population_noseeds, sum)

FrontRes_noseeds <- as.data.frame(t(fitness_noseeds))
fitness
...
Plot1 <- ggplot() +
  ...
  geom_point(data = FrontRes_noseeds, aes(x = FrontRes_noseeds[, 1], y = FrontRes_noseeds[, 2], col = "ECR"),
             size = 2) +
